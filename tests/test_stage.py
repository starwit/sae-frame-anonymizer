...
def set_config():
    with patch('frameanonymizer.stage.FrameAnonymizerConfig') as mock_config:
        def _make_mock_config(stream_id: str, input_stream_prefix: str):
            mock_config.return_value = FrameAnonymizerConfig(
                log_level='WARNING',
                redis=RedisConfig(
                    stream_id=stream_id,
...
def test_example(set_config, redis_publisher_mock, inject_consumer_messages):
    ...
    assert redis_publisher_mock.call_args_list[0].args == ('frameanonymizer:test_stream', _make_sae_msg_bytes(1))
    assert redis_publisher_mock.call_args_list[1].args == ('frameanonymizer:test_stream', _make_sae_msg_bytes(2))
